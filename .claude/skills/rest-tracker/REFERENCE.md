...
- **`fetch-chavesnamao.js`** (verified live 20/08/2026, no bot-blocking, plain `curl`) — every page (bairro list and individual listing alike) embeds one or more `<script type="application/ld+json">` blocks; the one that matters has `"@type":"RealEstateListing"`, either at the top level (list page) or inside a top-level `"@graph"` array (detail page) — `findRealEstateListing()` in the script handles both shapes. List page: `https://www.chavesnamao.com.br/casas-para-alugar/{uf}-{city}/{bairro-slug}/` — `offers.numberOfItems` is the *authoritative* count of real matches; `offers.itemListElement` is padded to ~15 with "similar" filler listings from other neighborhoods, filtered out by matching `itemOffered.address.addressLocality` against the requested bairro. Each surviving element already carries price/area/bathrooms/address/geo/url/advertiser name — enough to build the listing URL set. Detail page: `https://www.chavesnamao.com.br/imovel/{slug}/id-{portalListingId}/` — the `@graph` `RealEstateListing` node's `.about.offers` has `itemOffered` (full address, `geo`, `amenityFeature[]` filtered to `value===true`, `petsAllowed`, `numberOfBathroomsTotal`) and `offeredBy` (agent name, `telephone`, their chavesnamao profile URL — not their own site). No condo/IPTU field found anywhere in the JSON-LD; left `null`/"Não informado". **Quirk**: `itemOffered.numberOfBedrooms`/`numberOfRooms` are unreliable (seen as `"0"` on a listing whose free-text description said "02 Quartos") — the script regexes bedroom count (and parking spaces) out of the free-text `description` first and only falls back to the JSON field if that regex finds nothing.
